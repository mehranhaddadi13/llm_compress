...
*/
{
    ...
    unsigned int model, len, alphabet_size, escape_method;
    unsigned int start, step, stop;
    boolean performs_full_excls, performs_update_excls;
    int max_order;
...
unsigned int
TLM_create_PPM_model (char *title, unsigned int alphabet_size, int max_order, unsigned int escape_method, unsigned int performs_excls)
/* Creates a new empty dynamic PPM model. Returns the new model number allocated
   ...
   escape_method is the escape method being used by the model. e.g. 'C' for PPMC and 'D' for PPMD.
   performs_excls is TRUE if the model is to perform exclusions, otherwise it does not.
*/
{
    unsigned int model;

    model = TLM_create_model (TLM_PPM_Model, title, alphabet_size, max_order, escape_method, performs_excls);
    return (model);
}
