...
struct CPTc_trie_type { /* node in the trie */
  ...
  struct CPTc_trie_type *next;   /* the next link in the list */
  struct CPTc_trie_type *down;   /* the next level down in the trie */
  struct CPTp_table_type *table; /* the cumulative probability table of predictions
				   associated with this context */
};

struct CPTc_table_type {         /* stores context contexts and their cumulative probabilities tables of
				   associated predictions */
  struct CPTc_trie_type *trie;   /* pointer to trie of contexts */
  unsigned int types;           /* Number of unique contexts */
};

void
CPTc_init_table (struct CPTc_table_type *table);
...
boolean
CPTc_update_table (struct CPTc_table_type *ctable, unsigned int context,
		  unsigned int key, unsigned int freq, struct CPTp_table_type *keys);
