...
#define ALPHABET_SIZE 256 /* Default maximum alphabet size for all PPM models */
...
init_arguments (int argc, char *argv[])
{
    ...
    while ((opt = getopt (argc, argv, "1:2:L:VDM:a:o:d:l:m:p:")) != -1)
	switch (opt)
	{
	case '1':
...
{
    unsigned int textlen, symbol, prevsymbol, model, p;

    assert (TXT_valid_text (text));

    prevsymbol = TXT_sentinel_symbol ();
    model = NIL;
	...
	else if (prevsymbol >= TXT_sentinel_symbol ())
	  {
	    model = symbol;
	    fprintf (fp, "<%s>", TLM_get_tag (model));
	  }
...
void
encodeFile (unsigned int transform_model, unsigned int input_file)
/* Encodes the text from the file fp using the transform model and dynamic language model. */
{
    ...
    while ((model = TLM_next_modelno ()))
        TTM_start_transform (transform_model, TTM_transform_multi_context, input_text, model);
...
main (int argc, char *argv[])
{
    ...
    TLM_reset_modelno ();
    while ((model = TLM_next_modelno ()))
      TTM_add_transform (transform_model, 0.0, "%w", "%$%m%w", model); /* add transform for every static model */
    ...
    encodeFile (transform_model, Stdin_File);
