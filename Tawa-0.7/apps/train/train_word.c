/* Loads or creates a word-based model, adds some text to the model,
...
usage (void)
{
    fprintf (stderr,
	     ...
	     "  -5 n\tescape method of word model=c (optional; default=D)\n"
	     "  -6 n\tescape method of character model=c (optional; default=D)\n"
	     "  -7 n\tescape method of non-word model=c (optional; default=D)\n"
	     "  -8 n\tescape method of non-character model=c (optional; default=D)\n"
...
init_arguments (int argc, char *argv[])
{
    ...
    Title_found = FALSE;
    Debug.level = 0;
    while ((opt = getopt (argc, argv, "ST:d:m:p:t:1:2:3:4:5:6:7:8")) != -1)
	switch (opt)
	{
	case 'S':
	    Static_Model = TRUE;
	...
	case '7':
	    escape = optarg [0] - 'A';
	    assert (escape >= 0);
	    Nonword_model_escape_method = escape;
	    break;
	case '8':
	    escape = optarg [0] - 'A';
	    assert (escape >= 0);
	    Nonchar_model_escape_method = escape;
	    break;
	default:
	    usage ();
	    break;
...
main (int argc, char *argv[])
{
	...
	check_model (Nonword_model, Nonword_model_max_order);
	check_model (Nonchar_model, Nonchar_model_max_order);
      }
    else
      {
